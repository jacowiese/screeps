...
export class RangedDefender extends BaseCreep {
    ...
    public spawnCreep(creepName: string, spawn: StructureSpawn): void {
        ...
        let creepMemory: CreepMemory = {role: "RANGEDDEFENDER", state: "", room: spawn.room.name };

        let body: Array<BodyPartConstant> = new Array<BodyPartConstant>();

        let numParts = Math.floor(spawn.room.energyAvailable / 270);
        if (numParts == 0) {
            return;
        }
        ...
        switch (result) {
            case ERR_NOT_ENOUGH_ENERGY: {
                console.log('Could not spawn ranged-defender: not enough energy!');
            }
        }
    }
